...
- `MonitorForm` — borderless topmost per-pixel-alpha layered window (WS_EX_LAYERED + UpdateLayeredWindow, drawn to an ARGB bitmap): rounded dark card, mic glyph, status dot, drop shadow. All drawing is in 68px design units scaled by `_scalePercent` (right-click Scale menu: 50–200%). Left-drag moves it, left-click toggles mute, right-click menu (Scale / Re-authorize / Exit). White mic + green dot = unmuted, red mic + slash = muted/deafened, gray = disconnected.
